...
const InvestmentChart: React.FC<InvestmentChartProps> = ({ yearlyReturns }) => {
  ...
  if (yearlyReturns.length === 0) {
    return <div>{t(`investmentChart.dataNotFound`)}</div>;
  }
    ...
    <div className="relative w-full overflow-x-auto">
              ...
              <div key={idx} className="flex items-end" style={{ width: `${100 / yearlyReturns.length}%` }}>
                ...
                >
                  ...
                  <div className="absolute opacity-0 group-hover:opacity-100 bottom-full left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 mb-1 whitespace-nowrap">
                    {t('investmentChart.spentOnCigarettes', {
                      spent: formatCurrency(yr.cigaretteSpent)
                    })}
                  </div>
                </div>
                ...
                <div
                  ...
                  title={
                    t('investmentChart.investmentValue', {
                      investmentValue: formatCurrency(yr.investmentValue)
                    })}
                >
                  {/* Tooltip on hover */}
      ...
      <div className="flex justify-center mt-8">
        <div className="flex items-center mr-6">
          ...
          <span className="text-sm text-gray-600">{t('investmentChart.spentOnCigarettesTotal')}</span>
        </div>
        <div className="flex items-center">
          <div className="w-4 h-4 bg-green-500 rounded-sm mr-2"></div>
          <span className="text-sm text-gray-600">{t('investmentChart.investmentValueTotal')}</span>
        </div>
      </div>
    </div>
  );
